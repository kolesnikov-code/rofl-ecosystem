...
async def notify_new_user(telegram_id: int, eco_id: str, username: str, first_name: str, last_name: str,
    ...
    try:
        ...
        text = (
            ...
            f"• <b>ROFL ID:</b> <code>{safe_eco_id}</code>\n"
        ...
        logger.info(f"✅ Уведомление для {telegram_id} (ROFL ID: {eco_id})")
    except Exception as e:
        logger.error(f"❌ Ошибка уведомления: {e}")

async def notify_channel_subscription(telegram_id: int, username: str, first_name: str, last_name: str,
                                      channel_name: str, channel_username: str, action: str, emoji: str = "✅"):
    try:
        safe_username = html.escape(username)
        safe_first = html.escape(first_name)
        safe_last = html.escape(last_name)
        ...
        await bot.send_message(chat_id=ADMIN_CHANNEL_ID, text=text, parse_mode="HTML")
        logger.info(f"📢 {action} на {channel_username} от {telegram_id}")
    except Exception as e:
        logger.error(f"❌ Ошибка уведомления о подписке: {e}")